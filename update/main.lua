...
-- Baixa o repositório do GitHub e instala em /storage/roms/ports/
...
local WORKER = string.format([[
  ...
  ch:push("downloading")
  exec("wget -q '" .. REPO_URL .. "' -O '" .. TMP_ZIP .. "'")
  ...
  exec("unzip -q '" .. TMP_ZIP .. "' -d '" .. TMP_DIR .. "'")
  ...
  ch:push("installing")
  exec("cp -r '" .. TMP_DIR .. "/tetris-master/.' '" .. PORTS .. "/'")

  ch:push("cleanup")
  exec("rm -rf '" .. TMP_ZIP .. "' '" .. TMP_DIR .. "'")
...
function love.draw()
  ...
  -- steps
  local ix = 200   -- x do ícone
  local lx = 228   -- x do label
  local sy = 138   -- y inicial
  local sg = 50    -- gap entre steps

  for i, step in ipairs(STEPS) do
    local y  = sy + (i - 1) * sg
    local iy = y + 10   -- centro vertical do ícone
